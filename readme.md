...
#### BUG 9 — `tas` byte order reversed vs. every other single-operand MAP3 instruction **FIXED (July 2026, live test pending)**

**Verified:** debug-session bad-instruction log at `0x14656` (bytes `17 f8 12 3c e6 07 8c`). All eight
`tas` EA-form constructors in `h8539f-arith.sinc` (lines 707–770) were written as:

```sleigh
:tas    eab_direct      is opcode_special=0x17; eab_direct { ... }
```

i.e. `opcode_special` **before** the EA subtable — backwards compared to every sibling
single-operand instruction in the same file (`tst`, `clr`, `neg`, `shal`, `shar`, `shll`, `shlr`,
`rotl`, `rotr`, `rotxl`, `rotxr`), which all correctly use `is eab_direct; opcode_special=0xNN`
(EA byte first, dispatch byte second).

Because `tas`'s pattern didn't require an EA-shaped byte 1, SLEIGH treated any lone byte `0x17`
anywhere in the stream as a valid start of a `tas` instruction, then required the *next* byte to
satisfy `eab_direct`'s `mode=10 & sz=0`. At `0x14656`, byte 2 (`0xf8`) decodes as `mode=15, sz=1`,
which matches none of `tas`'s byte-only (`sz=0`) EA forms — hence the resolve failure.

**Fix (applied, all 8 forms):** swapped clause order to match `tst`'s proven pattern, e.g.:

```sleigh
:tas    eab_direct      is eab_direct;      opcode_special=0x17 { ... }
```

Applied to `eab_direct`, `eab_indirect`, `eab_disp8`, `eab_disp16`, `eab_predec`,
`eab_postinc`, `eab_abs8_br`, `eab_abs16`. Not yet confirmed by a fresh re-import + full
auto-analysis — do that next and check `0x14656` decodes as `tas` before marking this closed.
